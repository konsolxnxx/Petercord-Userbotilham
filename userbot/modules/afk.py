...
# ========================= CONSTANTS ============================
AFKSTR = [
    f"**Maaf 𝐏𝐨𝐜𝐨𝐧𝐠 𝐎𝐧𝐥𝐞𝐧 User {ALIVE_NAME} Sedang AFK!**",
    f"**Maaf 𝐏𝐨𝐜𝐨𝐧𝐠 𝐎𝐧𝐥𝐞𝐧 User {ALIVE_NAME} Sedang AFK\n Tunggu Sampai Dia Kembali Online! Jangan rindu ya berat**",
    f"**𝐏𝐨𝐜𝐨𝐧𝐠 𝐎𝐧𝐥𝐞𝐧 {ALIVE_NAME} Sedang AFK\n Tunggulah Sampai Online**",
    f"**Maaf 𝐏𝐨𝐜𝐨𝐧𝐠 𝐎𝐧𝐥𝐞𝐧 User {ALIVE_NAME} Sedang AFK!**",
]
...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit(f"**✘ AFK!**\n**𝐏𝐨𝐜𝐨𝐧𝐠 𝐎𝐧𝐥𝐞𝐧 User AFK**\
        \n☛ **Alasan:** `{string}`")
    else:
        await afk_e.edit("**✘ AFK!**\n**𝐏𝐨𝐜𝐨𝐧𝐠 𝐎𝐧𝐥𝐞𝐧 Telah AFK**")
    if user.last_name:
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("**𝐏𝐨𝐜𝐨𝐧𝐠 𝐎𝐧𝐥𝐞𝐧 User Telah Kembali!**")
        time.sleep(3)
        await msg.delete()
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"**✘ 𝐏𝐨𝐜𝐨𝐧𝐠 𝐎𝐧𝐥𝐞𝐧 {ALIVE_NAME} Sedang AFK** {afk_since} **Yang Lalu.**\
                        \n☛ **Alasan:** `{AFKREASON}`")
                else:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"**✘ 𝐏𝐨𝐜𝐨𝐧𝐠 𝐎𝐧𝐥𝐞𝐧 Masih AFK** {afk_since} **Yang Lalu.**\
                            \n☛ **Alasan:** `{AFKREASON}`")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"✘ **𝐏𝐨𝐜𝐨𝐧𝐠 𝐎𝐧𝐥𝐞𝐧 Sedang AFK** {afk_since} **Yang Lalu**.\
                        \n☛ **Alasan**: `{AFKREASON}`")
                else:
